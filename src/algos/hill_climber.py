...
class HillClimber(Algorithm):
    ...
    @time_it
    def solve(self, distance_matrix: pd.DataFrame, starting_solution: Union[list, np.ndarray] = None, num_iter: int=50, return_tuple: bool=False) -> Union[Tuple[int, str], int]:
    ...
    def solve_multistart(self, distance_matrix: pd.DataFrame, num_iter: int=20, num_starts: int=10) -> Tuple[int, str]:
        ...
        for i in range(num_starts):
            results.append(self.solve(distance_matrix, num_iter=num_iter, return_tuple=True))
            print(f"Start {i}")
    ...
    def solve_multistart_parallel(self, distance_matrix: pd.DataFrame, num_iter: int=20, num_starts: int=10) -> Tuple[int, str]:
        """Solve TSP problem with Hill Climber Algorithm with multiple starts
    ...
    def _find_best_neighbour(self, neighbours:np.ndarray, **kwargs) -> np.array:
        ...
        """
        return neighbours[np.argmin([self._compute_distance(neighbour, **kwargs) for neighbour in neighbours])]
    ...
    def get_distances(self, neighbours:np.array, **kwargs) -> pd.Series:
        ...
        """
        return pd.Series({self._compute_distance(neighbour, **kwargs): neighbour for neighbour in neighbours})
